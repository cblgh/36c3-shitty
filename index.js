...
const URL = 'http://data.c3voc.de/36C3/everything.schedule.xml'
const CACHE = path.join(os.homedir(), '.36c3', 'schedule.xml')
let activeCol = 0
let grid, talk
...
function help () {
  console.log('Usage: 36c3 [options]')
  console.log()
  console.log('Options:')
  ...
  fs.stat(CACHE, function (err) {
    ...
    fs.readFile(filepath, function (err, xml) {
      ...
      try {
        ...
        console.error('Run "36c3 --update" to re-download the schedule')
        process.exit(1)
      }
    })
  })
...
function initMenu (schedule) {
  ...
  schedule.day.forEach(function (day, index) {
    ...
    day.room.forEach(function (room, roomIndex) {
      ...
      const color = tracks.get(track) || function (a) { return a }
...
function updateTopBar () {
  grid.update(0, 0, renderTopBar(` 36c3 schedule - ${chalk.bold('enter:')} details, ${chalk.bold('space:')} favorite, ${chalk.bold('tab:')} switch column`, activeCol === 0))
